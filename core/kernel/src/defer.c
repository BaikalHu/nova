...
static int __deferred_put (uintptr_t arg1, uintptr_t arg2)
    ...
    /* it is ok inovke this before deferred task init as TASK_STATUS_READY is 0 */

    if ((__defer->status != TASK_STATUS_READY) &&
        (__defer->wanted.pdata == NULL))
        {
        __defer->wanted.pdata = (void *) job;
...
static int __deferred_get (uintptr_t arg1, uintptr_t arg2)
    ...
    if (__deferred_head == NULL)
        ...
        {
        __defer->wanted.pdata = (void *) __deferred_head;
...
static __noreturn int deferred_task (uintptr_t dummy)
    ...
    uintptr_t                   arg;
    deferred_job_t            * job;
    deferred_job_t * volatile * next = (deferred_job_t * volatile *) &__defer->wanted.pdata;
